...
import { groupKeyMap, categoryKeyMap } from "@/i18n/translations";
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="max-w-6xl mx-auto space-y-4">
          {categories.map((category) => {
            const catKey = categoryKeyMap[category.name];
            return (
              <div
                key={category.name}
                  ...
                  <div className={`${category.textColor} px-6 md:px-8 pb-6 md:py-6`}>
                    <Accordion type="multiple" className="space-y-0">
                      {[...category.groups].sort((a, b) => a.name.localeCompare(b.name)).map((group) => {
                        const gKey = groupKeyMap[group.name];
                        return (
                          <AccordionItem
                            key={group.name}
                            value={group.name}
                          ...
                          >
                            <AccordionTrigger className="py-5 hover:no-underline group">
                              <span className="font-display text-lg md:text-xl font-medium text-left">
                                {gKey ? t(`${gKey}.name`) : group.name}
                              </span>
                            </AccordionTrigger>
                            <AccordionContent className="pb-5">
                              <p className="text-base opacity-90 leading-relaxed">
                                {gKey ? t(`${gKey}.desc`) : group.description}
                              </p>
                              {group.link && (
                                <a
                                  href={group.link}
